      ...
      <div class="table-responsive">
        <table style="text-align: center;" class="table table-hover">
          ...
          <tbody>
            <tr class="cosa">
              @foreach ($data as $img)
              @if($img->name == 'secondcarousel1')
              <th scope="row">SLIDER 1 </th>
              <td><img style="width: 15em; height: 10em;" src="{{asset('storage/slider')}}/{{$img->file_path}}"></td>
              <td><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal4">
                  Cambiar
                </button></td>
              @endif
              @endforeach
            ...
            <tr>
              @foreach ($data as $img)
              @if($img->name == 'secondcarousel3')
              <th scope="row">SLIDER 3 </th>
              <td><img style="width: 15em; height: 10em;" src="{{asset('storage/slider/')}}/{{$img->file_path}}"></td>
              <td><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal6">
                  Cambiar
                </button></td>
              @endif
              @endforeach
